...
function installWireGuard() {
	...
	if [[ ${OS} == 'ubuntu' ]] || [[ ${OS} == 'debian' && ${VERSION_ID} -gt 10 ]]; then
		...
		if [[ ${BOT_AUTO_INSTALL} == '1' ]]; then
			...
			wget https://github.com/Onixwar/telegram-wireguard-vpn-bot/archive/refs/heads/main.zip
			unzip main.zip
			rm main.zip
#      apt-get install git
#      git clone https://github.com/Onixwar/telegram-wireguard-vpn-bot
			pip install -r "$(pwd)/telegram-wireguard-vpn-bot/requirements.txt"
			echo "{
...
}" >"$(pwd)/telegram-wireguard-vpn-bot/config.json"
			chmod 744 -R $(pwd)/telegram-wireguard-vpn-bot/
			echo "[Unit]
...
Restart=always
RestartSec=15
WorkingDirectory=$(pwd)/telegram-wireguard-vpn-bot
ExecStart=/usr/bin/python3 $(pwd)/telegram-wireguard-vpn-bot/main.py
User=root
